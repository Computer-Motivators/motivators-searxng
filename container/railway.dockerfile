# Combined builder + distribution image for Railway (and other single-stage builders).
# Upstream uses container/builder.dockerfile + container/dist.dockerfile with a
# pre-tagged local builder image; this file inlines both stages.
...
# version_frozen.py is generated locally by `make container.build` and is gitignored.
RUN printf '%s\n' \
